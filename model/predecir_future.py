...
FUTURE_STEPS_TO_PREDICT = 30 # ¿Cuántos días hacia el futuro predecir? (ej: 90 días ~ 3 meses)
...
if __name__ == "__main__":
    ...
    try:
        model.load_state_dict(torch.load(f"modelos/{MODEL_PATH}", map_location=device))
    ...
    if len(df) < SEQ_LENGTH:
        print(f"Error: No hay suficientes datos históricos ({len(df)}) para formar la secuencia inicial de longitud {SEQ_LENGTH}.")
        exit()
    ...
    current_sequence_tensor = torch.tensor(last_known_sequence_scaled, dtype=torch.float32).unsqueeze(0).to(device)
    print(f"Forma tensor secuencia inicial: {current_sequence_tensor.shape}")  # Debería ser (1, 60, 2)

    # 5. Realizar Predicción Iterativa
    future_predictions_scaled = []
    print(f"Iniciando predicción iterativa para {FUTURE_STEPS_TO_PREDICT} días futuros...")

    with torch.no_grad():
        for i in range(FUTURE_STEPS_TO_PREDICT):
            # Predecir el siguiente paso (solo el precio)
            next_pred_scaled_tensor = model(current_sequence_tensor)  # Salida shape (1, 1)
            ...
            last_sma_scaled = (df["SMA"].iloc[-1] - scaler.center_[2]) / scaler.scale_[2]
            
            # Crear tensor con TRES características
            next_step_scaled = torch.tensor([
                [next_pred_scaled_tensor.item(), neutral_rsi_scaled, last_sma_scaled]
    ...
    future_predictions_for_inverse = np.zeros((len(future_predictions_scaled), len(FEATURES)))
    future_predictions_for_inverse[:, 0] = future_predictions_scaled  # Precio
    future_predictions_for_inverse[:, 1] = 50  # RSI neutral
    future_predictions_for_inverse[:, 2] = df["SMA"].iloc[-1]  # Último SMA conocido

    future_predictions = scaler.inverse_transform(future_predictions_for_inverse)[:, 0]  # Tomar solo el precio
    ...
    df_future = pd.DataFrame({'Predicción': future_predictions.flatten()}, index=future_dates)
    ...
    plt.savefig(f"images/prediccion/{MODEL_PATH}.png", dpi=300, bbox_inches='tight')
