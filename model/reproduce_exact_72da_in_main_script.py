#!/usr/bin/env python3
"""
reproduce_exact_72da_in_main_script.py
Reproduce EXACTAMENTE 72.41% DA usando el script principal pero con configuración exacta
"""
...
def set_exact_seed_and_run():
    """Configurar entorno exacto y ejecutar"""
    
    print("🎯 REPRODUCIENDO EXACTAMENTE 72.41% DA EN SCRIPT PRINCIPAL")
    print("=" * 70)
    
    # Crear script temporal con configuración exacta
    script_content = '''
import numpy as np
import pandas as pd
import torch
...
print()
print("🎯 RESULTADOS EXACTOS SCRIPT PRINCIPAL:")
print("=" * 50)
...
results = {
    ...
    'rmse': float(rmse),
    'r2': float(r2),
    'target_da_original': target_da,
    'target_da_objective': target_70,
    'objective_70_achieved': da >= target_70,
    'exact_match': abs(da - target_da) < 0.1,
...
results_file = f"exact_72da_main_script_{timestamp}.json"
...
'''
    ...
    # Escribir script temporal en el directorio correcto
    temp_script_path = Path('model') / 'temp_exact_72da.py'
    with open(temp_script_path, 'w', encoding='utf-8') as f:
        f.write(script_content)
    
    print(f"📝 Script temporal creado: {temp_script_path}")
    print("🚀 Ejecutando configuración exacta...")
    
    # Ejecutar script
    try:
        result = subprocess.run([
            'C:/Users/rony1/OneDrive/Documents/GitHub/LSTM-EUR-USD/.venv/Scripts/python.exe',
